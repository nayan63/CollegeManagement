...
public class CollegeImplementation implements CollegeService {
	...
	public List<ResponseTemplate> listOfStudent() {
		List<ResponseTemplate> RTList = new ArrayList<ResponseTemplate>();
		List<College> colList = collegeRepo.findAll();
		Iterator<College> ir = colList.iterator();
		while(ir.hasNext())
		{
			College college = ir.next();
			List<Student> stuList = restTemplate.getForObject("http://STUDENT-SERVICE/student/student-by-name/"+college.getId(), List.class);
			ResponseTemplate RT = new ResponseTemplate();
			RT.setCollege(college);
			RT.setStudentList(stuList);
			RTList.add(RT);
		}
